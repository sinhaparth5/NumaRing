...
\title{\Large\bfseries NumaRing: Topology-Aware Routing for NUMA-Local MPMC Queues,\\
and What Broke When We Optimized It}
\author{
NumaRing Project\\
\small \href{https://github.com/sinhaparth5/NumaRing}{github.com/sinhaparth5/NumaRing}
}
\date{}
...
work stealing, built on the bounded MPMC algorithm of Vyukov \citep{vyukov_bounded_mpmc}, itself a
descendant of the Michael--Scott queue \citep{michael1996simple}; (2) a profiling-driven fix to a
...
\textbf{Lock-free MPMC queues.} Michael and Scott's non-blocking queue \citep{michael1996simple}
established the two-pointer, compare-and-swap-based design most later concurrent queues build on.
Vyukov's bounded MPMC queue \citep{vyukov_bounded_mpmc} adapts that idea to a fixed-size ring
...
\texttt{boost::lockfree::queue} \citep{boost_lockfree} and moodycamel's
\texttt{ConcurrentQueue} \citep{desrochers2014moodycamel} are both mature, widely used C++ MPMC
queues; neither is NUMA-topology-aware, and we use both as baselines in
Section~\ref{sec:eval}.

\textbf{NUMA-aware synchronization.} David et al.\ \citep{david2013everything} is the closest
...
\textbf{Detecting cross-socket contention.} \texttt{perf c2c} \citep{mario2016c2c} instruments the
...
\citep{vyukov_bounded_mpmc}. Every slot holds a value and its own atomic sequence number. An

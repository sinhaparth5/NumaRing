...
\end{tikzpicture}
\caption{NumaRing's two-node architecture. Producers and consumers route to their own node's ring
buffer via \texttt{current\_node()} (Section~\ref{sec:topology}); the fast path never crosses the
dashed boundary. Batched work stealing (Section~\ref{sec:worksteal}) only runs when a node's ring
is locally full or empty.}
\label{fig:arch}
\end{figure}
